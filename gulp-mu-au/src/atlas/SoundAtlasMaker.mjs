...
export class SoundAtlasMaker {
	...
	static async Create(_options = {}) {
		...
		const files = [];
		if (_options.src) files.push(...ListAudioFiles(_options.src));
		if (Array.isArray(_options.sounds)) files.push(..._options.sounds);
		...
		log(`gulp-mu-au: ${skipped ? "atlas up to date" : "built atlas"} '${dataFile}' (${sounds.length} sounds).`);
