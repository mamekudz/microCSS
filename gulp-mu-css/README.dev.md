...
In Entwicklung. Umgesetzt sind die Meilensteine **M1** (Core-Pipeline), **M2** (Sprites & Cursor), **M3** (Manifest & Build), **M4** (Hooks & Makros), **M5** (Legacy-Migration) und **M6** (Handbuch):
...
- **Farb-API** — `Lighten(color, step, model = "hsl")`, `Alpha(color, alpha)`, `MixColors(c1, c2)`, `ParseColor`, `FormatColor`. Das `"hsl"`-Modell ist bitgenau zur alten µCSS™-Implementierung (verifiziert gegen kompilierte Legacy-Ausgaben), `"oklch"` steht als wahrnehmungsgleichmäßige Alternative bereit.
...
Die Tests prüfen u. a. die Farbfunktionen gegen Werte, die die alte µCSS™-Toolchain in der kompilierten Legacy-`std.css` erzeugt hat (Bitgenauigkeit des `hsl`-Modells).
<!-- publish:exclude:end -->
